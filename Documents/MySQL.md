...
![](https://s2.ax1x.com/2020/01/17/lzylOH.png)
...
在进行查询时，索引的列不能是表达式的一部分，也不能是函数的参数，否则无法使用索引。
...
就是将大查询切分，因为不切分，一次执行，会耗尽资源，从而阻塞很多小的但重要的查询。
...
将一个大连接查询分解成单表查询，之后应用程序中进行关联，这样做的好处有：
...
水平切分成为Sharding，它是将同一个表中的记录拆分到相同结构的表中。
...
-   **I/O 线程**  ：负责读取主服务器的二进制日志，写入从服务器中（Relay log）。
...
- [b tree index and hash index](https://medium.com/@mena.meseha/what-is-the-difference-between-mysql-innodb-b-tree-index-and-hash-index-ed8f2ce66d69)
