...
- [01-基本架构:一个键值数据库包含什么?](#01-基本架构一个键值数据库包含什么)
- [02-数据结构:快速的Redis有哪些慢操作?](#02-数据结构快速的redis有哪些慢操作)
- [03-高性能IO模型:为什么单线程Redis能那么快?](#03-高性能io模型为什么单线程redis能那么快)
- [04-AOF日志:宕机了，Redis如何避免数据丢失?](#04-aof日志宕机了redis如何避免数据丢失)
- [05-内存快照:宕机后，Redis如何实现快速恢复?](#05-内存快照宕机后redis如何实现快速恢复)
- [06-数据同步:主从库如何实现数据一致?](#06-数据同步主从库如何实现数据一致)
- [07-哨兵机制:主库挂了，如何不间断服务?](#07-哨兵机制主库挂了如何不间断服务)
- [08-哨兵集群:哨兵挂了，主从库还能切换吗?](#08-哨兵集群哨兵挂了主从库还能切换吗)
- [09-切片集群:数据增多了，是该加内存还是加实例?](#09-切片集群数据增多了是该加内存还是加实例)
- [11-“万金油”的String，为什么不好用了?](#11-万金油的string为什么不好用了)
- [12-有一亿个keys要统计，应该用哪种集合？](#12-有一亿个keys要统计应该用哪种集合)
- [13-GEO是什么？还可以定义新的数据类型吗？](#13-geo是什么还可以定义新的数据类型吗)
- [14-如何在Redis中保存时间序列数据？](#14-如何在redis中保存时间序列数据)
- [15-消息队列的考验：Redis有哪些解决方案？](#15-消息队列的考验redis有哪些解决方案)
- [16-异步机制:如何避免单线程模型的阻塞?](#16-异步机制如何避免单线程模型的阻塞)
- [17-为什么CPU结构也会影响Redis的性能?](#17-为什么cpu结构也会影响redis的性能)
- [18-波动的响应延迟:如何应对变慢的Redis?(上)](#18-波动的响应延迟如何应对变慢的redis上)
- [19-波动的响应延迟：如何应对变慢的Redis？（下）](#19-波动的响应延迟如何应对变慢的redis下)
- [20-删除数据后，为什么内存占⽤率还是很⾼？](#20-删除数据后为什么内存占率还是很)
- [21-缓冲区：⼀个可能引发“惨案”的地⽅](#21-缓冲区个可能引发惨案的地)
- [23-旁路缓存：Redis是如何⼯作的？](#23-旁路缓存redis是如何作的)
- [24-替换策略：缓存满了怎么办？](#24-替换策略缓存满了怎么办)
- [25-缓存异常（上）：如何解决缓存和数据库的数据不⼀致问题？](#25-缓存异常上如何解决缓存和数据库的数据不致问题)
- [26-缓存异常（下）：如何解决缓存雪崩、击穿、穿透难题？](#26-缓存异常下如何解决缓存雪崩击穿穿透难题)
- [27-缓存被污染了，该怎么办？](#27-缓存被污染了该怎么办)
- [28-Pika-如何基于SSD实现⼤容量Redis？](#28-pika-如何基于ssd实现容量redis)
- [29-⽆锁的原⼦操作：Redis如何应对并发访问？](#29-锁的原操作redis如何应对并发访问)
- [30-如何使⽤Redis实现分布式锁？](#30-如何使redis实现分布式锁)
- [31-事务机制：Redis能实现ACID属性吗？](#31-事务机制redis能实现acid属性吗)
- [32-Redis主从同步与故障切换，有哪些坑？](#32-redis主从同步与故障切换有哪些坑)
- [33-脑裂：⼀次奇怪的数据丢失](#33-脑裂次奇怪的数据丢失)
- [35-CodisVSRedisCluster:我该选择哪一个集群方案](#35-codisvsrediscluster我该选择哪一个集群方案)
- [36-Redis支撑秒杀场景的关键技术和实践都有哪些?](#36-redis支撑秒杀场景的关键技术和实践都有哪些)
- [37-数据分布优化:如何应对数据倾斜?](#37-数据分布优化如何应对数据倾斜)
- [38-通信开销:限制RedisCluster规模的关键因素](#38-通信开销限制rediscluster规模的关键因素)
- [39-Redis6.0的新特性:多线程、客戶端缓存与安全](#39-redis60的新特性多线程客戶端缓存与安全)
- [40-Redis的下一步:基于NVM内存的实践](#40-redis的下一步基于nvm内存的实践)
- [Redis的使用规范](#redis的使用规范)
...
## 25-缓存异常（上）：如何解决缓存和数据库的数据不⼀致问题？

1. 如何解决数据不⼀致问题？
2. 先删除缓存，再更新数据库会发生什么？
3. “延迟双删“知道么？
4. 先更新数据库值，再删除缓存值呢？
5. 先更新数据库，再更新缓存，写+写并发会发生什么？
6. 先更新缓存，再更新数据库，写+读并发呢？

## 26-缓存异常（下）：如何解决缓存雪崩、击穿、穿透难题？

1. 你能分别解释一下缓存雪崩、缓存击穿和缓存穿透的现象么？
2. 那该如何解决这些问题呢？
...
## 28-Pika-如何基于SSD实现⼤容量Redis？

1. Pika这个开源库知道么？
2. 把Redis数据迁移到Pika中的命令是什么？

## 29-⽆锁的原⼦操作：Redis如何应对并发访问？

1. Redis的两种原⼦操作⽅法知道么？

## 30-如何使⽤Redis实现分布式锁？

1. 每个线程加锁时设置随机值，释放锁时判断是否和加锁设置的值⼀致，为什么要判断一下？
...
1. 不合理配置项导致的服务挂掉这个知道么？
2. 怎样可以查看主库接收写命令的进度信息？

## 33-脑裂：⼀次奇怪的数据丢失

1. 导致Redis主库⽆法响应⼼跳了这种情况有遇到么？
2. 有两个配置项来限制主库的请求处理，知道么？
...
1. 秒杀场景的负载特征对支撑系统的要求？
2. 基于分布式锁来支撑秒杀场景如何处理呢？

## 37-数据分布优化:如何应对数据倾斜?

## 38-通信开销:限制RedisCluster规模的关键因素

## 39-Redis6.0的新特性:多线程、客戶端缓存与安全
